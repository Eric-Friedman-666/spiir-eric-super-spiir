...
if [ "$dependencies_bool" = false ]; then
  ...
  if [ "$ozstar_bool" = true ]; then
    ...
    psd="${dependencies_dir}/H1L1V1-REFERENCE_PSD-1186624818-511200.xml.gz"; else
    ...
    frame_cache="${dependencies_dir}/frame_for_unit_test.cache"
    psd="${dependencies_dir}/H1L1V1-REFERENCE_PSD-1186624818-511200.xml.gz"
  fi
fi
...
macro_iir_all_banks="H1:${dependencies_dir}/iir_H1-GSTLAL_SPLIT_BANK_0003-a1-0-0.xml.gz,L1:${dependencies_dir}/iir_L1-GSTLAL_SPLIT_BANK_0003-a1-0-0.xml.gz,V1:${dependencies_dir}/iir_V1-GSTLAL_SPLIT_BANK_0003-a1-0-0.xml.gz"
detrsp_map="${dependencies_dir}/H1L1V1_detrsp_map_${event_time}.xml"

echo """gstlal_inspiral_postcohspiir_online \
	--job-tag 000 \
	...
	--cuda-postcoh-detrsp-fname $detrsp_map --cuda-postcoh-detrsp-refresh-interval 86400 \
	...
	--finalsink-far-factor 2 --finalsink-singlefar-veto-thresh 0.5 \
    ...
    --cuda-postcoh-parti-ifos H1L1 \
	# --nxydump 118700881:1187008883 \
	--verbose"""
