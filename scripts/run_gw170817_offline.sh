...
#
#   On ozstar:
# default frame file is: $dir_containing_script/frame_cache_OzStar_O2_C00.cache
...
if [ "$dependencies_bool" = false ]; then
  ...
  if [ "$ozstar_bool" = true ]; then
    ...
    frame_cache=/fred/oz016/gwdc_spiir_pipeline_codebase/gwdc_utils/unit_testing/dependencies/frame_cache_OzStar_O2_C00.cache
	#$(dirname "$0")/dependencies/frame_cache_OzStar_O2_C00.cache
  else
...
frame_cache=${frame_cache:-${dependencies_dir}/frame_cache_OzStar_O2_C00.cache}

echo "reading dependencies from: ${dependencies_dir}"

macro_iir_all_banks="H1:${dependencies_dir}/iir_H1-GSTLAL_SPLIT_BANK_${bankid}-a1-0-0.xml.gz,L1:${dependencies_dir}/iir_L1-GSTLAL_SPLIT_BANK_${bankid}-a1-0-0.xml.gz,V1:${dependencies_dir}/iir_V1-GSTLAL_SPLIT_BANK_${bankid}-a1-0-0.xml.gz"
