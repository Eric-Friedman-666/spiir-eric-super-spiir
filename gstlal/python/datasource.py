...
state_vector_on_off_dict = {
	...
	"L1" : [0x7, 0x160],
	"V1" : [0x67, 0x100],
	"K1" : [0x67, 0x100] # TODO: Test framexmit source with Kagra enabled/disabled, see !36
}
...
def mkbasicsrc(pipeline, gw_data_source_info, instrument, nxydump_segment = None, nxydump_directory = '.', verbose = False):
	...
	if gw_data_source_info.data_source == "white":
		...
		if instrument == "V1":
			...
			src = pipeparts.mklalcachesrc(pipeline, location = gw_data_source_info.frame_cache, cache_src_regex = "V") # NOTE: Investigate how KAGRA frames are named
		else:
