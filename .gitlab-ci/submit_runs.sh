...
export JOB_ID=${JOB_ID:-$(git rev-parse --abbrev-ref HEAD | sed -E 's/\//__/g')-$(git rev-parse --short HEAD)}
...
while getopts a:s:d:j:w:l:p:g:i:x:y flag
do
    case "${flag}" in
...
MAIN_COMMIT=$(basename $(ls -td ${ARTIFACTS_DIR}/spiir-O4-EW-development*/ | head -1 || echo spiir-O4-EW-development))
...
for idx in "${!IFOS_LIST[@]}"; do
  ...
  mkdir -p "${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs"
  ...
  CMD3="python3 $SCRIPT_DIR/compare_runs.py --control ${MAIN_COMMIT} --test ${JOB_ID} --ifos ${RUN_ID}"
  if [ $SLURM == 1 ]; then
    SCMD="sbatch --export=ALL --job-name=${START}-${DURATION}-${JOB_ID}-${RUN_ID} --parsable --output=${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt --error=${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt --open-mode=append"
    ...
    echo $CMD
    bash $CMD 1>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt 2>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt &
    pid=$!
    tail --pid=$pid -F ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt
		echo $CMD2
		$CMD2 1>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt 2>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt &
    pid=$!
    tail --pid=$pid -F ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt
		echo $CMD3
		$CMD3 1>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt 2>> ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt &
    pid=$!
    tail --pid=$pid -F ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.out.txt ${ARTIFACTS_DIR}/${JOB_ID}/${RUN_ID}/logs/pipe-run.err.txt
  fi
done
