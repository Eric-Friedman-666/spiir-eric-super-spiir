...
Requires: gstlal >= 1.0.0 python >= 3.8 glue >= 1.49 glue-segments >= 1.49 python-pylal >= @MIN_PYLAL_VERSION@ fftw >= 3 %{gstreamername} >= 1.14.1 %{gstreamername}-plugins-base >= 1.14.1 %{gstreamername}-plugins-good >= 0.10.27 gstreamer_lscsoft-python >= 0.10.21 pygobject2 numpy scipy lal >= 7.1.0 lalmetaio >= 2.0.0 gsl ldas-tools-framecpp >= @MIN_FRAMECPP_VERSION@ gds-core >= @MIN_GDS_VERSION@ gds-crtools >= @MIN_GDS_VERSION@ gds-services >= @MIN_GDS_VERSION@ cyrus-sasl-lib nds2-client >= @MIN_NDS_VERSION@ orc >= 0.4.16
BuildRequires: doxygen graphviz gstlal-devel >= 1.0.0 python-devel >= 3.8 fftw-devel >= 3 %{gstreamername}-devel >= 1.14.1 %{gstreamername}-plugins-base-devel >= 1.14.1 pygobject2-devel lal-devel >= 7.1.0 lalmetaio-devel >= 2.0.0 gsl-devel ldas-tools-framecpp-devel >= @MIN_FRAMECPP_VERSION@ gds-devel >= @MIN_GDS_VERSION@ cyrus-sasl-devel nds2-client-devel >= @MIN_NDS_VERSION@ nds2-client-headers >= @MIN_NDS_VERSION@ orc >= 0.4.16
...
Group: LSC Software/Data Analysis
Requires: %{name} = %{version} gstlal-devel >= 1.0.0 python-devel >= 3.8 fftw-devel >= 3 %{gstreamername}-devel >= 1.14.1 %{gstreamername}-plugins-base-devel >= 1.14.1 pygobject2-devel lal-devel >= 7.1.0 lalmetaio-devel >= 2.0.0 gsl-devel nds2-client-headers >= @MIN_NDS_VERSION@
%description devel
