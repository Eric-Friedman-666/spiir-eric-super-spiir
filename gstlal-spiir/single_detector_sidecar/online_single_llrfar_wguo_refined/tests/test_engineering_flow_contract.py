...
class EngineeringFlowContractTests(unittest.TestCase):
    ...
    def test_crashcar_env_uses_common_noninject_inject_sections(self) -> None:
        ...
        self.assertIn("injection_bg_zerolag_update_hour=", inject_block)
        self.assertIn("injection_file=", inject_block)
        self.assertIn("injection_BG_accumulation_hour=", inject_block)
        self.assertIn("injection_zerolag_update_hour=", inject_block)
    ...
    def test_crashcar_frozen_injection_workflow_uses_lower_data_contract(self) -> None:
        ...
        self.assertIn("FROZEN_MULTI_FALLBACK_DIR=${injection_bg_seed_noninj_stats_loc", source)
        self.assertIn('materialize_frozen_multi_stats "${BG_RUN_ROOT}/run" "${INJ_WORKERS}" "${BG_WORKERS}" "${FROZEN_MULTI_DIR}" "${FROZEN_MULTI_FALLBACK_DIR}"', source)
        self.assertIn('fallback_src="${fallback_dir}/${src_jobno}/${src_jobno}_marginalized_stats_${suffix}.xml.gz"', source)
        ...
        self.assertIn("BG_ACCUM_SECONDS=${injection_bg_BG_accumulation_seconds", source)
        self.assertIn("BG_ACCUM_HOUR=${injection_bg_BG_accumulation_hour", source)
        self.assertIn("BG_ZL_UPDATE_SECONDS=${injection_bg_zerolag_update_seconds", source)
        self.assertIn("INJ_ACCUM_SECONDS=${injection_BG_accumulation_seconds:-${injection_accumulation_seconds:-}}", source)
        self.assertIn("INJ_ACCUM_HOUR=${injection_BG_accumulation_hour:-${injection_accumulation_hour:-}}", source)
        self.assertIn("INJ_ACCUM_SECONDS=${BG_ACCUM_SECONDS}", source)
        self.assertIn("INJ_ZL_UPDATE_SECONDS=${injection_zerolag_update_seconds:-}", source)
        self.assertIn("INJ_ZL_UPDATE_HOUR=${injection_zerolag_update_hour:-}", source)
        self.assertIn("INJ_ZL_UPDATE_SECONDS=${BG_ZL_UPDATE_SECONDS}", source)
