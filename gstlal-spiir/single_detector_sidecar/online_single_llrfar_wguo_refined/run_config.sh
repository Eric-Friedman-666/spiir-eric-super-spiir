...
export SPIIR_RUNTIME_GST_PLUGIN_PATH=${SPIIR_RUNTIME_GST_PLUGIN_PATH:-}
export SPIIR_RUNTIME_LD_LIBRARY_PATH=${SPIIR_RUNTIME_LD_LIBRARY_PATH:-}
...
export UPDATE_INTERVAL_SECONDS=${UPDATE_INTERVAL_SECONDS:-1}
...
export SINGLE_INPUT_KIND=${SINGLE_INPUT_KIND:-singlecsv}
export FINAL_SINGLE_INPUT_KIND=${FINAL_SINGLE_INPUT_KIND:-zerolag}
export FINAL_SINGLE_RESET_LEDGER=${FINAL_SINGLE_RESET_LEDGER:-0}
export FINAL_SINGLE_IGNORE_ONLINE_REPLAY_GATE=${FINAL_SINGLE_IGNORE_ONLINE_REPLAY_GATE:-1}
export SINGLE_TRIGGER_STREAM_ENABLE=${SINGLE_TRIGGER_STREAM_ENABLE:-1}
export SINGLE_TRIGGER_STREAM_FILE=${SINGLE_TRIGGER_STREAM_FILE:-}
export SIDECAR_PRESERVE_TABLE_SINGLE_FAR=${SIDECAR_PRESERVE_TABLE_SINGLE_FAR:-1}
export SIDECAR_SINGLE_FAR_LEDGER=${SIDECAR_SINGLE_FAR_LEDGER:-}
export SIDECAR_SINGLE_FAR_LOOKUP_INTERVAL_SECONDS=${SIDECAR_SINGLE_FAR_LOOKUP_INTERVAL_SECONDS:-1.0}
export SIDECAR_PATCH_ZEROLAG_SINGLE_FAR=${SIDECAR_PATCH_ZEROLAG_SINGLE_FAR:-1}
export PATCH_ZEROLAG_SINGLE_FAR=${PATCH_ZEROLAG_SINGLE_FAR:-${SIDECAR_PATCH_ZEROLAG_SINGLE_FAR}}
export PATCH_ZEROLAG_SINGLE_FAR_LEDGER=${PATCH_ZEROLAG_SINGLE_FAR_LEDGER:-single_branch/single_final_far_all.csv}
export PATCH_ZEROLAG_SINGLE_FAR_COLUMN=${PATCH_ZEROLAG_SINGLE_FAR_COLUMN:-direct_far}
export SINGLE_OUTPUT_MODE=${SINGLE_OUTPUT_MODE:-single-only}
export SINGLE_OUTPUT_ACTIVE_IFO_SCHEDULE=${SINGLE_OUTPUT_ACTIVE_IFO_SCHEDULE:-}
export PATCH_ZEROLAG_SINGLE_OUTPUT_MODE=${PATCH_ZEROLAG_SINGLE_OUTPUT_MODE:-${SINGLE_OUTPUT_MODE}}
export PATCH_ZEROLAG_SINGLE_OUTPUT_ACTIVE_IFO_SCHEDULE=${PATCH_ZEROLAG_SINGLE_OUTPUT_ACTIVE_IFO_SCHEDULE:-${SINGLE_OUTPUT_ACTIVE_IFO_SCHEDULE}}
export PREFER_FEATURE_SINGLE_FAR=${PREFER_FEATURE_SINGLE_FAR:-0}
export CRASHCAR_ENABLE=${CRASHCAR_ENABLE:-0}
export CRASHCAR_DETAIL_OUTPUT_FNAME=${CRASHCAR_DETAIL_OUTPUT_FNAME:-}
export CRASHCAR_LOG10_FAR_THRESHOLD=${CRASHCAR_LOG10_FAR_THRESHOLD:--4.0}
export CRASHCAR_SNR_SERIES_OUTPUT_DIR=${CRASHCAR_SNR_SERIES_OUTPUT_DIR:-crashcar_snr_series}
...
# accidentally fall back to the system gstlal_spiir Python package.
if [ "${CRASHCAR_ENABLE}" = "1" ]; then
    export CRASHCAR_INSTALL_ROOT=${CRASHCAR_INSTALL_ROOT:-$(cd "${PACKAGE_ROOT}/.." && pwd)/install}
    export SPIIR_ONLINE_BIN=${SPIIR_ONLINE_BIN:-${CRASHCAR_INSTALL_ROOT}/bin/gstlal_inspiral_postcohspiir_online}
    export SPIIR_RUNTIME_PYTHONPATH=${SPIIR_RUNTIME_PYTHONPATH:-${CRASHCAR_INSTALL_ROOT}/lib/python3.10/site-packages}
    export SPIIR_RUNTIME_GST_PLUGIN_PATH=${SPIIR_RUNTIME_GST_PLUGIN_PATH:-${CRASHCAR_INSTALL_ROOT}/lib/gstreamer-1.0}
    export SPIIR_RUNTIME_LD_LIBRARY_PATH=${SPIIR_RUNTIME_LD_LIBRARY_PATH:-${CRASHCAR_INSTALL_ROOT}/lib}
    if [ -z "${CRASHCAR_TEMPLATE_SHAPE_MAP_FNAME:-}" ] && [ -n "${RUN_DIR:-}" ]; then
        export CRASHCAR_TEMPLATE_SHAPE_MAP_FNAME=${RUN_DIR}/crashcar_template_shape_map.csv
    fi
fi
