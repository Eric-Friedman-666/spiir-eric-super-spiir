...

{
    for name in \
        ...
        PATCH_ZEROLAG_SINGLE_SNR_SERIES PATCH_ZEROLAG_SINGLE_SNR_SERIES_MANIFEST \
        ...
        CRASHCAR_ENABLE CRASHCAR_PRESERVE_TABLE_SINGLE_FAR CRASHCAR_DETAIL_OUTPUT_FNAME CRASHCAR_LOG10_FAR_THRESHOLD CRASHCAR_SNR_SERIES_OUTPUT_DIR CRASHCAR_MIN_SNR \
        CRASHCAR_FAR_FLOOR_COUNT CRASHCAR_LIVETIME_STEP CRASHCAR_BACKGROUND_REQUIRED_SECONDS \
        CRASHCAR_EXPECTED_BUFFERS_PER_TIMESTAMP CRASHCAR_TEMPLATE_SHAPE_MAP_FNAME CRASHCAR_REQUIRE_TEMPLATE_SHAPE_MAP \
...
run_final_single_update() {
    ...
    if [ "${PATCH_ZEROLAG_SINGLE_FAR:-${SIDECAR_PATCH_ZEROLAG_SINGLE_FAR:-1}}" = "1" ]; then
        ...
        if [ "${PATCH_ZEROLAG_SINGLE_SNR_SERIES:-${CRASHCAR_ENABLE:-0}}" = "1" ]; then
            patch_args+=(
                --embed-snr-series
                --snr-series-manifest "${PATCH_ZEROLAG_SINGLE_SNR_SERIES_MANIFEST:-${CRASHCAR_SNR_SERIES_OUTPUT_DIR:-crashcar_snr_series}/manifest.csv}"
            )
        fi
