...
def add_feature_csv_arguments(parser):
    ...
    parser.add_argument(
        ...
        help=("LIGO-LW segment XML/XML.GZ used to compute per-IFO online "
              "livetime inside the background window; may be repeated"))
    parser.add_argument(
...
def calculate_single_detector_rows_from_feature_csv(
    ...
    if segment_xml_files:
        if background_start_gps is None or background_end_gps is None:
            raise ValueError(
                "--background-start-gps and --background-end-gps are required "
                "when --segment-xml is used")
        segment_maps = [
            load_ligolw_segment_xml(path)
            for path in segment_xml_files
        ]
        all_segments = merge_segment_maps(segment_maps)
        for ifo in ifos:
            segment_livetimes[ifo] = segment_livetime_seconds(
                all_segments, ifo, background_start_gps, background_end_gps)

    if background_features:
        default_livetime = background_livetime
        if default_livetime is None:
            default_livetime = infer_feature_livetime_seconds(background_features)
        if not segment_livetimes:
            if default_livetime is None or float(default_livetime) <= 0.0:
                raise ValueError(
                    "background livetime is required to assign FAR from LLR")
        for ifo in ifos:
            if any(feature.ifo == ifo for feature in background_features):
                livetime = segment_livetimes.get(ifo, default_livetime)
                if livetime is not None and float(livetime) > 0.0:
                    branch.add_livetime(livetime, [ifo],
                                        gps=background_end_gps)
        if calibrate_noise_dof:
            branch.rebuild_background_support(background_features)
        else:
