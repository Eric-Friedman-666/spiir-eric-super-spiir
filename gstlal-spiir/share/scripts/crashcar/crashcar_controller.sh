...
SOURCE_ROOT=${SOURCE_ROOT:?SOURCE_ROOT required in ${CONFIG_FILE}}
CRASH_RUNTIME_ROOT=${CRASH_RUNTIME_ROOT:-"${ROOT}/crashcar_runtime"}
...
GITHUB_BRANCH=${GITHUB_BRANCH:-Eric-bless-spiir-crashcar}
GITHUB_REF="refs/heads/${GITHUB_BRANCH}"

START_GPS=${START_GPS:?START_GPS required in ${CONFIG_FILE}}
if [ -z "${END_GPS:-}" ]; then
    DURATION=${DURATION:?DURATION required when END_GPS is unset}
    END_GPS=$((START_GPS + DURATION))
fi
DURATION=$((END_GPS - START_GPS))
BACKGROUND_ACCUMULATION=${BACKGROUND_ACCUMULATION:-${BACKGROUND_ACCUMULATION_SECONDS:-10800}}
BACKGROUND_UPDATE=${BACKGROUND_UPDATE:-${BACKGROUND_UPDATE_TRIGGER_SECONDS:-3600}}
SNAPSHOT_INTERVAL=${SNAPSHOT_INTERVAL:-${BACKGROUND_UPDATE}}
WORKER_COUNT=${WORKER_COUNT:-2}
BANKS_PER_WORKER=${BANKS_PER_WORKER:-8}
START_BANK=${START_BANK:-0}

SEGMENT_XML=${SEGMENT_XML:?SEGMENT_XML required in ${CONFIG_FILE}}
LIVETIME_CSV=${LIVETIME_CSV:-"${ARTIFACTS}/H1L1V1_SEGMENTS_${START_GPS}_${DURATION}_livetime.csv"}
DETRSP_MAP=${DETRSP_MAP:?DETRSP_MAP required in ${CONFIG_FILE}}
FRAME_CACHE=${FRAME_CACHE:?FRAME_CACHE required in ${CONFIG_FILE}}
NONINJ_STATS_LOC=${NONINJ_STATS_LOC:?NONINJ_STATS_LOC required in ${CONFIG_FILE}}
O3_BANK_DIR=${O3_BANK_DIR:?O3_BANK_DIR required in ${CONFIG_FILE}}
WGUO_BANK_STATS_DIR=${WGUO_BANK_STATS_DIR:?WGUO_BANK_STATS_DIR required in ${CONFIG_FILE}}
...
CRASHCAR_CODE_VERSION=${CRASHCAR_CODE_VERSION:-"spiir-crashcar-${GITHUB_BRANCH}"}
SLURM_JOB_NAME=${SLURM_JOB_NAME:-crashcar}
...
TMUX_SESSION=${TMUX_SESSION:-codex1}

H_ONLY_SECONDS=${H_ONLY_SECONDS:-0}
L_ONLY_SECONDS=${L_ONLY_SECONDS:-0}
HL_SECONDS=${HL_SECONDS:-0}
HL_NONE_SECONDS=${HL_NONE_SECONDS:-0}
SINGLE_ONLY_SECONDS=${SINGLE_ONLY_SECONDS:-0}
SINGLE_ONLY_FRACTION=${SINGLE_ONLY_FRACTION:-}
HL_UNION_FRACTION=${HL_UNION_FRACTION:-}
FIRST3_H_ONLY_SECONDS=${FIRST3_H_ONLY_SECONDS:-0}
FIRST3_L_ONLY_SECONDS=${FIRST3_L_ONLY_SECONDS:-0}
FIRST3_HL_SECONDS=${FIRST3_HL_SECONDS:-0}
FIRST3_HL_NONE_SECONDS=${FIRST3_HL_NONE_SECONDS:-0}
