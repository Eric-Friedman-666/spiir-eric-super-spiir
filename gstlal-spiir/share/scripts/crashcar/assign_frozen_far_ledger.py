...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dof", type=float, default=sdf.DEFAULT_EFFECTIVE_DOF)
    ...
    parser.add_argument("--beta-grid-size", type=int, default=10)
    ...
    parser.add_argument(
        "--prefer-feature-single-far",
        action="store_true",
        default=os.environ.get("PREFER_FEATURE_SINGLE_FAR", "0") == "1",
        help=("when a detector-local feature row already carries assigned "
              "single-detector FAR values, preserve those as the authoritative "
              "final FAR instead of overwriting them with this Python ledger"))
...
def assignment_window_end(feature: sdf.SingleDetectorFeature,
    ...
    if snapshot_times is not None:
        ...
        if (gps is not None and update > 0.0 and float(gps) < start
            ...
            # before the snapshot start.  Online assignment handles those rows
...
def neg_log10_text(value: float) -> str:
    if value <= 0.0 or not math.isfinite(value):
        return ""
    return "%.12g" % (-math.log10(value))


def feature_single_far(feature: sdf.SingleDetectorFeature) -> tuple[float, str] | None:
    source = feature.source_row if isinstance(feature.source_row, dict) else {}
    explicit = finite_positive_number(source.get("far_sngl"))
    if explicit is not None:
        return explicit, "crashcar_c_far_sngl"

    candidates: list[tuple[float, str]] = []
    for field, label in (
            ("far_1w", "crashcar_c_far_1w_sngl"),
            ("far_1d", "crashcar_c_far_1d_sngl"),
            ("far_2h", "crashcar_c_far_2h_sngl")):
        value = finite_positive_number(source.get(field))
...
def preserve_feature_single_far(row: dict[str, str],
                                feature: sdf.SingleDetectorFeature,
                                args: argparse.Namespace) -> None:
    if not args.prefer_feature_single_far:
        return
    selected = feature_single_far(feature)
    if selected is None:
        return
    far, source = selected
    far_value = float_text(far)
    neg_log10 = neg_log10_text(far)
    row["far"] = far_value
    row["far_source"] = source
    row["neg_log10_far"] = neg_log10
    row["assigned_far"] = far_value
    row["assigned_far_source"] = source
    row["assigned_neg_log10_far"] = neg_log10
...
def assign_with_fixed_background(args: argparse.Namespace,
    ...
    for feature in foreground_features:
        ...
        for row in rows:
            preserve_feature_single_far(row, feature, args)
    ...
    summary = {
        ...
        "prefer_feature_single_far": args.prefer_feature_single_far,
    }
...
def main() -> int:
    ...
    for index, end in enumerate(sorted_group_ends):
        ...
        for row, feature in zip(rows, groups[end]):
            preserve_feature_single_far(row, feature, args)
