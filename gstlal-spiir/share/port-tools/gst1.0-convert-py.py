...
# Copied from https://git.ligo.org/lscsoft/gstlal/-/blob/master/gstlal/port-tools/gst1.0-convert-py.py
...
with open(sys.argv[1]) as f:
    for line in f:
        ...
        if import_gobject_line_no > 0 and (line_no -
                                           import_gobject_line_no) > 5:
            mod_str = ' # MOD: Error: found import gobject but expected import gst to be within [' + str(
                gobject_gst_lineno_diff) + '] lines.'
            modded = True
        ...
        if m != None:
            if gobject_loaded == True:
                ...
                gobject_postload = m.group(
                    1) + 'GObject.threads_init()' + '\n' + m.group(
                        1) + 'Gst.init(None)'
            continue
        ...
        if m != None:
            ...
            if m2 != None:
                ...
                (mod_line,
                 mod_num) = re.subn(r'gst\s*\.\s*Pipeline\s*\(([^\)]+)\)',
                                    r'gst.Pipeline(name=\1)', line)
                if mod_num != 0:
                    line = mod_line
                    modded = True
        ...
        if m != None:
            ...
            # Yes: more than one argument
            if m2 != None:
                (mod_line, mod_num) = re.subn(r'element_factory_make',
                                              r"ElementFactory.make", line)
                if mod_num != 0:
                    line = mod_line
                    modded = True
            # No: only one argument, add a ", None"
                ...
                (mod_line,
                 mod_num) = re.subn(r'element_factory_make\s*\((.+)\)',
                                    r"ElementFactory.make(\1, None)", line)
                if mod_num != 0:
                    line = mod_line
        ...
        # gst.event_new_seek() -> Gst.Event.new_seek()
        # (from Chad)
        (mod_line, mod_num) = re.subn(r'event_new_seek\s*\((.*)\)',
                                      r"Event.new_seek(\1)", line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        if m != None:
            mod_str = ' # MOD: Error line [' + str(
                line_no
            ) + ']: element_link_many not yet implemented. See web page **'
            modded = True
        ...
        if m != None:
            ...
            m2 = re.search('\,', m.group(1))
            if m2 != None:
                mod_str = ' # MOD: Found an add with multiple args: [' + str(
                    line) + ' ], args = [' + str(m.group(1)) + ']'
                modded = True
        ...
        # From Chad:
        # gst.STATE_CHANGE_FAILURE -> Gst.StateChangeReturn.FAILURE
        (mod_line, mod_num) = re.subn(r'STATE_CHANGE_FAILURE',
                                      r'StateChangeReturn.FAILURE', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        (mod_line, mod_num) = re.subn(r'STATE_CHANGE_SUCCESS',
                                      r'StateChangeReturn.SUCCESS', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'MESSAGE_ELEMENT',
                                      r'MessageType.ELEMENT', line)
        if mod_num != 0:
            line = mod_line
            modded = True

        (mod_line, mod_num) = re.subn(r'MESSAGE_APPLICATION',
                                      r'MessageType.APPLICATION', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'gst\s*\.\s*GhostPad',
                                      r'Gst.GhostPad.new', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        #        if string.startswith('audio/'):
        #            <link to some audio elements>
        (mod_line, mod_num) = re.subn(r'\.\s*get_caps\s*[\(\)]*\s*\[\s*0\s*\]',
                                      r'.query_caps(None)', line)
        if mod_num != 0:
            line = mod_line
            modded = True

        (mod_line, mod_num) = re.subn(r'\.\s*get_caps\s*[\(\)]*',
                                      r'.query_caps(None)', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'\.\s*get_pad\s*\(',
                                      r'.get_static_pad(', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'get_negotiated_caps',
                                      r'get_current_caps', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'gst\s*\.\*query_new_duration',
                                      r'Gst.Query.new_duration', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        if m != None:
            mod_str = ' # MOD: Found type_register in line: [' + str(
                line) + ']'
            modded = True

        m = re.search('element_register', line)
        if m != None:
            modstr = ' # MOD: Found element_register in line: [' + str(
                line) + ']'
            modded = True
        ...
        (mod_line, mod_num) = re.subn(r'\.\s*PAD_SRC', r'.PadDirection.SRC',
                                      line)
        if mod_num != 0:
            line = mod_line
            modded = True
        (mod_line, mod_num) = re.subn(r'\.\s*PAD_SINK', r'.PadDirection.SINK',
                                      line)
        if mod_num != 0:
            line = mod_line
            modded = True
        (mod_line, mod_num) = re.subn(r'\.\s*PAD_ALWAYS',
                                      r'.PadPresence.ALWAYS', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        (mod_line, mod_num) = re.subn(r'\.\s*PAD_REQUEST',
                                      r'.PadPresence.REQUEST', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        (mod_line, mod_num) = re.subn(r'\.\s*PAD_SOMETIMES',
                                      r'.PadPresence.SOMETIMES', line)
        if mod_num != 0:
            line = mod_line
            modded = True
        ...
        if m != None:
            mod_str = ' # MOD: Found ffmpegcolorspace in line: [' + str(
                line) + ']'
            modded = True
